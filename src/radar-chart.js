var RadarChart = {
  defaultConfig: {
    ...
    tooltipFormatValue: function(d, stat) {
      return d + " - " + stat;
    },
  ...
  chart: function() {
    ...
    function setTooltip(tooltip, msg, num, option){
      if(msg === false || msg == undefined){
        ...
        if(option == 'pokemon') {
          imgName = num;
          imgHref = "./img/"+option+"/"+imgName+".png"
        }
        else if(option == 'types') {
          imgName = msg.toLowerCase()+"Icon";
          imgHref = "./img/"+option+"/"+imgName+".png"
        }
        else {
          imgSize = 0;
        ...
        tooltip.select("text").classed('visible', 1).attr("style", "fill: "+cfg.tooltipColor+"; transform: translateX("+(imgSize+padding).toString()+"px)");
        ...
        .classed("visible", 1)
        .attr("href", imgHref)
        .attr("height", imgSize.toString()+"px")
        .attr("width", imgSize.toString()+"px")
        .attr("style", "transform: translateY(-"+(imgSize-padding).toString()+"px)");

        tooltip.select("rect")
        .classed('visible', 1).attr("x", 0)
        .attr("x", bbox.x - padding)
        .attr("y", bbox.y - padding)
        .attr("width", bbox.width + imgSize + (padding*times))
    ...
    function radar(selection) {
      selection.each(function(data) {
        ...
        tt.append('image').classed("tooltip", true);
        ...
        .on('mouseover', function (dd){
          ...
          setTooltip(tooltip, cfg.tooltipFormatClass(dd.className), dd.number, option);
        })
        ...
        .each(function(d, i) {
          ...
          if(d.className) {
            d.className = d.className.replace(':','-_');
        ...
        if(cfg.circles && cfg.radius) {
          ...
          .on('mouseover', function(dd){
            d3.event.stopPropagation();
            setTooltip(tooltip, cfg.tooltipFormatValue(dd[0].value, dd[0].axis), -2, 'circles');
          })
